...
describes what users get. Pick a format with `-o`, or let clevelandart choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
clevelandart <command> -o table   # aligned columns for reading
clevelandart <command> -o jsonl   # one JSON object per line, for piping
clevelandart <command> -o json    # a single JSON array
clevelandart <command> -o csv     # spreadsheet friendly
clevelandart <command> -o tsv     # tab-separated
clevelandart <command> -o url     # just the URL column
clevelandart <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
clevelandart <command> --fields id,title,url
```
...
```bash
clevelandart <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
clevelandart <command>            # a table, because this is a terminal
clevelandart <command> | wc -l    # JSONL, because this is a pipe
```
